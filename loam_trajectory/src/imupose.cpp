...
Imuposeinf::Imuposeinf()
{
    ...
    gn << 0, 0, -9.7919;
    ba << 0.01, 0.01, 0.01;
    bw << 0.0016, 0.0013, 0.0024;//第三代0.0016, 0.0013, 0.0024
    q_pre.w() = 1.0;
    q_pre.x() = 0.0;
    q_pre.y() = 0.0;
    q_pre.z() = 0.0;
    ...
    mOmegaEarth = 7.292115e-5;    ;
    mLongitude = 112.9916*M_PI/180;
    mLatitude = 28.2202*M_PI/180;

    imu_sub = nhimu.subscribe<sensor_msgs::Imu>("/imu/data", 50, &Imuposeinf::imuCallback, this);
    //loam_sub = nhloam.subscribe<nav_msgs::Odometry>("/integrated_to_init", 2, &Imuposeinf::loamCallback, this);
...
void Imuposeinf::loamCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
